...
from pyrogram.types import Message, InlineKeyboardButton, InlineKeyboardMarkup, InputMediaAudio, InputMediaVideo
...
async def song_commad_private(client, message: Message, _):
    if message.chat.type != ChatType.PRIVATE:
        button = InlineKeyboardMarkup([[InlineKeyboardButton(_["SG_B_1"], url=f"https://t.me/{app.username}")]])
        await message.reply_text(_["song_12"], reply_markup=button)
        return
...
async def song_helper_cb(client, CallbackQuery, _):
    ...
    if stype == "audio":
        ...
        keyboard.append([
            InlineKeyboardButton(text=_["BACK_BUTTON"], callback_data=f"song_back {vidid}"),
            InlineKeyboardButton(text=_["CLOSE_BUTTON"], callback_data="close")
        ])
        return await CallbackQuery.edit_message_reply_markup(InlineKeyboardMarkup(keyboard))
    else:
        try:
            formats_available, link = await YouTube.formats(vidid, True)
...
async def song_download_cb(client, CallbackQuery, _):
    try:
        await CallbackQuery.answer("ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ...", show_alert=True)
    except:
        pass
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    ...
    with yt_dlp.YoutubeDL({"quiet": True, "user-agent": "Mozilla/5.0 (Linux; Android 7.0; k960n_mt6580_32_n) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.101 Safari/537.36"}) as ytdl:
