...
async def server_to_play(client, CallbackQuery, _):
    ...
    what = (CallbackQuery.data.strip()).split(None, 1)[1]
    mystic = await CallbackQuery.edit_message_text(_["tracks_1"].format(what, user_name))
    upl = failed_top_markup(_)
    if what == "Global":
        stats = await get_global_tops()
    elif what == "Group":
        stats = await get_particulars(CallbackQuery.message.chat.id)
    elif what == "User":
    ...
    if not stats:
        return await mystic.edit_caption(_["tracks_2"].format(what), reply_markup=upl)
