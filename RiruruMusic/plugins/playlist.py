...
async def check_playlist(client, message: Message, _):
    ...
    if link:
        ...
        ran_hash = f"\\cache\Playlist{message.chat.id}{message.from_user.id}.txt"
        with open(ran_hash, "w") as pl:
            pl.write(msg)
        try:
            await message.reply_document(ran_hash, caption=_["playlist_4"], file_name="Playlist.txt")
            await get.delete()
...
async def get_keyboard(_, user_id):
    ...
    for x in _playlist:
        ...
        title = _note["title"].title()
        keyboard.append([InlineKeyboardButton(text=title, callback_data=f"del_playlist {x}")])
    keyboard.append([
...
async def play_playlist(client, CallbackQuery, _):
    ...
    try:
        ...
        try:
            await AltCall.join_assistant(chat_id, chat_id)
            await stream(
                _,
                mystic,
                user_id,
...
async def add_playlist(client, CallbackQuery, _):
    ...
    try:
        return await CallbackQuery.message.reply_text(_["playlist_10"].format(CallbackQuery.from_user.mention))
    except:
        return
