...
from RiruruMusic.utils.database import (
    ...
    blacklisted_chats, get_assistant, get_lang, get_userss, is_on_off
)
...
async def start_comm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if name[0:4] == "help":
            ...
            m = await message.reply_text(f"> ɢᴇᴛᴛɪɴɢ ʏᴏᴜʀ ᴩᴇʀsᴏɴᴀʟ sᴛᴀᴛs ғʀᴏᴍ ʙᴏᴛ's sᴇʀᴠᴇʀ.")
            stats = await get_userss(message.from_user.id)
            tot = len(stats)
            if not stats:
                await asyncio.sleep(1)
                return await m.edit_text(_["ustats_1"])
        ...
        try:
            await message.reply_photo(
                ...
                caption=_["start_2"].format(
                    message.from_user.first_name, app.mention, served_users, served_chats
                ),
                reply_markup=InlineKeyboardMarkup(out),
            )
        except:
            await message.reply_text(
                _["start_2"].format(message.from_user.first_name, app.mention, served_users, served_chats),
                reply_markup=InlineKeyboardMarkup(out),
            )
        if await is_on_off(config.LOG):
            sender_id = message.from_user.id
